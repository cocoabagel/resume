...
- **実施内容**:
  - 注文確認画面のSwiftUIへのリニューアルと各項目の編集機能実装
...
#### XCTest、QuickのテストコードをSwift Testingへ完全移行(2025年下期)
- **課題**: XCTest、Quickのレガシーテストコードの排除
- **実施内容**:
  - Claude Codeを利用し役300件近いテストコードの変換を実施
  - ある程度の範囲でグループ分けしてAIによる変換を実施
  - ビルドが通り、ビルドワーニングも出なくなるところまでAIとペアプロ
  - 最終的な成果物について漏れがないか人力チェックで対応
  - Quickフレームワークの削除とObjective-Cを0%に
...
### 開発ツール・環境
- **Xcode**: 14 〜 26、Beta版での先行検証してユーザー一般リリース当日までにアプリ更新を実現
...
4. **技術選定力**: HLS vs mp4、CocoaPods vs SPMなど、要件に応じた適切な技術選定と社内合意
5. **先を見通す力**: iOS26（Liquid Glass）対応など、1年先を見据えた技術戦略の立案を主導
...
### チーム貢献
1. **技術リーダーシップ**: 毎朝のコードレビュー会、技術的な質問への回答とフォローアップ、オープンなPull Reqesut数を0件を実現
